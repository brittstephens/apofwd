...
#def get_ClusterClient(memory="25GB"): ## Jesse added project=
def get_ClusterClient(memory='25GB', project='NEOL0007'):
    ...
    cluster = PBSCluster(
        ...
        processes=1,
        queue='casper',
        local_directory=f'/glade/derecho/scratch/{USER}/dask-workers',
        log_directory=f'/glade/derecho/scratch/{USER}/dask-workers',
#        resource_spec='select=1:ncpus=1:mem=25GB', ## Jesse had dynamic memory
        resource_spec=f'select=1:ncpus=1:mem={memory}',
        project=project,
        walltime='06:00:00',
        interface='ext', # crashed without, trying others
        # 'ValueError: 'ib0' is not a valid network interface. Valid network interfaces are: ['lo', 'mgt', 'ext', 'eno2']
    ) # interface='ib0',) ## Jesse had this commented out
...
def get_rmask_dict(grid, mask_definition, plot=False):
    ...
    elif mask_definition == 'SO_Arctic':
        rmasks = dict(
            NH=grid.areacello.where(grid[lat_varname] >= 60.).fillna(0.),
            SH=grid.areacello.where(grid[lat_varname] < -60.).fillna(0.),
        ) 
...
def compute_fgapo(ds):
    ...
    # fgo2 and fgco2 are positive down, while fgn2 is positive up, thus adding instead of subtracting fgn2 here
    # compute_regional_integral above converts fgo2, fgco2, and fgapo to be positive up
